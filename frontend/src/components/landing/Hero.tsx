...
const ORB = "/4594a171-5e3b-40f0-a725-e4e246f02f09.jpg";
...
export function Hero() {
  return (
    <section className="relative overflow-hidden border-b border-line bg-cream pt-32 sm:pt-40">
      <div className="pointer-events-none absolute inset-x-0 top-0 hidden h-full hairline-grid opacity-60 lg:block" />
      ...
      <div className="relative mx-auto grid max-w-6xl items-center gap-16 px-6 pb-20 lg:grid-cols-[1.05fr_0.95fr] lg:gap-12 lg:pb-28">
        <div>
          ...
          <motion.h1
            ...
            className="mt-7 font-display text-[clamp(3rem,7.5vw,5.25rem)] font-normal leading-[0.95] tracking-[-0.02em] text-ink">
            
            Your AI,
            <br />
            <span className="italic text-peach">amplified.</span>
          </motion.h1>
          ...
          <motion.p
            ...
            Mavis is a multimodal assistant that searches the live web, reads your files, writes
            code, and keeps every conversation organized in one warm, quiet workspace.
          </motion.p>
          ...
          <motion.dl
            ...
            {[
            { value: 'Two', label: 'model providers' },
            { value: '9', label: 'file formats read' },
            { value: '0', label: 'setup steps' }].
            map((stat) =>
            <div key={stat.label}>
                <dt className="font-display text-2xl text-ink">{stat.value}</dt>
                <dd className="mt-1 font-mono text-[11px] uppercase tracking-[0.14em] text-muted">
                  {stat.label}
                </dd>
              </div>
            )}
